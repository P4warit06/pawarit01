...
public class Employee {
    ...
    public  Employee (String name , String lname    , int age , int id ){
        this.fname = fname;
        this.lname = lname;
        this.id = id;
        if (Utils059.checkInt(age)) this.age = age;
    ...
    public void setSalary(int salary) {
        if (Utils059.checkInt(salary)) this.salary = salary;
    }
    ...
    public void setBonus(int bonus) {
        if (Utils059.checkInt(bonus)) this.bonus = bonus;
    }
...
//30.1 While initialize the age in constructor use 'checkInteger' method in 'MathFuncYYY' class. throw 'IllegalArgumentException' error with the error message 'age must be positive'31. Create Getter and Setter for all attribute except 'fname', 'lname', 'age' and 'id';
